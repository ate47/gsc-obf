...
namespace gscobf::fastfile {
    ...
    FastfileInfo DecompressFastfile(void* file, size_t fileSize) {
        utils::bytebuffer::ByteBuffer reader{ (byte*)file, fileSize };

        if (!reader.CanRead(sizeof(data::fastfile::XFileBO3))) {
        ...
        while (offset < decompressedSize) {
            ...
            data::fastfile::DBStreamHeader* block{ reader.ReadPtr<data::fastfile::DBStreamHeader>() };
    ...
    void CompressFastfile(std::vector<byte>& out, FastfileInfo& info) {
        ...
        while (remainingSize) {
            ...
            data::fastfile::DBStreamHeader block;
